...
class Painting_task:
    def __init__(self, inventory):
        ...
        self.__bg = pygame.transform.smoothscale(pygame.image.load(config.painting_task_image).convert_alpha(), (self.__width, config.bg_height))        
        
        self.__font = pygame.font.Font(None, 30)
        self.__text = self.__font.render(config.painting_task_text, True, config.text_color)
        self.__text_false = self.__font.render("НЕВЕРНО!", True, (255, 0, 0))
        self.__text_true = self.__font.render("ВЕРНО!", True, (0, 255, 0))
        self.__text_one = self.__font.render(config.painting_task_text_one, True, config.text_color)
        self.__text_two = self.__font.render(config.painting_task_text_two, True, config.text_color)
        self.__text_three = self.__font.render(config.painting_task_text_three, True, config.text_color)
        self.__text_four = self.__font.render(config.painting_task_text_four, True, config.text_color)
